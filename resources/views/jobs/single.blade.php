...
<section class="section-hero overlay inner-page bg-image" style="background-image: url({{ asset('assets/images/hero_1.jpg') }});margin-top:-24px" id="home-section">
   ...
   @if (session()->has('create'))
       <p class="container alert alert-success text-center mt-2">{{ session('create') }}</p>
   @endif
   @if (session()->has('save'))
       <p class="container alert alert-danger text-center mt-2">{{ session('save') }}</p>
   @endif
   @if (session()->has('apply'))
       <p class="container alert alert-success text-center mt-2">{{ session('apply') }}</p>
   @endif
          ...
          <div class="row mb-5">
            <div class="col-6">
                ...
                <form action="{{ route('save.job') }}" method="POST">
                    ...
                    @if ($savedJob > 0)
                    <button name="submit" type="submit" class="btn btn-block btn-success btn-md disabled">You already saved this Job</button>
                    @else
            ...
            <div class="col-6">
                <form action="{{ route('apply.job') }}" method="POST">
                    ...
                    @if (isset(auth()->user()->id))
                    @if ($appliedJob > 0)
                        <button type="submit" name="submit" class="btn btn-block btn-secondary btn-md disabled">You already applied for this job</button>
                        @else
                        <button type="submit" name="submit" class="btn btn-block btn-primary btn-md">Apply Now</button>
                        @endif
                        @else
                        <a href="{{ route('login') }}" class="btn btn-block btn-warning btn-md">You should login in to apply or save this job</a>
                    @endif
                    </form>
            </div>
          </div>
    ...
    <div class="container">
      ...
      <div class="row mb-5 justify-content-center">
        <div class="col-md-7 text-center">
          <h2 class="section-title mb-2">{{ $jobCount }} Related Jobs</h2>
        </div>
      </div>
      ...
      <ul class="job-listings mb-5">
        ...
        @empty
            <p class="text-danger">No other jobs for this category</p>
        @endforelse

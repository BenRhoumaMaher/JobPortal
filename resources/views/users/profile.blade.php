...
<section class="section-hero overlay inner-page bg-image" style="background-image: url({{ asset('assets/images/hero_1.jpg') }}); margin-top: -24px" id="home-section">
    <div class="container">
        @if (session()->has('updateprofile'))
            <p class="container alert alert-success text-center text-white w-50">{{ session('updateprofile') }}</p>
        @endif
        {{-- @if (session()->has('updatecv'))
            <p class="container alert alert-success text-center text-white w-50">{{ session('updatecv') }}</p>
        @endif --}}
    <div class="row d-flex justify-content-center">
        ...
        <div class="card p-3 py-4">
                ...
                <div class="text-center mt-3">
                    ...
                    <h5 class="mt-2 mb-0"><strong>{{ $profile->name }}</strong></h5>
                    <span>{{ $profile->job_title }}</span><br>
                    <a href="{{ asset('assets/cvs/'.$profile->cv.'') }}" class="btn btn-info btn-block btn-lg w-50">Download CV</a>
                    <div class="px-4 mt-1">
